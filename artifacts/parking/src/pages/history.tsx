...
import { Search, History, Car, Bike, Truck } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Complete record of all parking sessions.</p>
      </div>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            <div className="flex gap-2 flex-1 min-w-[200px]">
              <Input placeholder="Search vehicle or owner…" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && setSearch(searchInput)} />
              <Button onClick={() => setSearch(searchInput)} size="sm">Search</Button>
            </div>
            ...
            {(search || startDate || endDate) && (
              <Button variant="outline" size="sm" onClick={() => { setSearch(""); setSearchInput(""); setStartDate(""); setEndDate(""); }}>Clear</Button>
            )}
          </div>
        </CardContent>
      </Card>
                ...
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                      ...
                      <p className="text-sm mt-1">Transactions will appear here after vehicles exit.</p>
                    </TableCell>
                  </TableRow>
                ) : (
                  transactions?.map((tx) => {
                    ...
                    return (
                      <TableRow key={tx.id}>
                        ...
                        <TableCell><Badge variant="outline" className="font-mono text-xs">{tx.slotNumber || `#${tx.slotId}`}</Badge></TableCell>
                        ...
                        <TableCell className="text-sm text-muted-foreground">{durationHrs > 0 ? `${durationHrs}h ` : ""}{durationMins}m</TableCell>
                        <TableCell className="text-right font-bold text-primary">₹{tx.fee.toFixed(2)}</TableCell>
                      </TableRow>
                    );
                  })
                )}
